...
var adjustment = module.exports = {
    ...
    do_adjustment:   function(data, adjust_mode) {
	    ...
	    if (adjust_mode == 'UseGoogle') {
	        ...
	        this.fixedShiftAdjustment(data, 0, data.distance.length - 1);
	...
	fixedShiftPartitionedAdjustment:   function(data) {
	    ...
	    for (end = 1; end < data.uploadElevation.length; end++) {
	        ...
	        if (!distanceDelta) {    
	            ...
	            var horizDistance = Math.sqrt(Math.abs(Math.pow(distanceDelta,2) - Math.pow(elevationDelta,2)));
	            var grade = 100 * elevationDelta / horizDistance;
	            
	            if (grade > adjustment.REASONABLE_GRADE_THRESHOLD || grade < adjustment.REASONABLE_GRADE_THRESHOLD * -1) {
	    ...
	    this.fixedShiftAdjustment(data, start, data.uploadElevation.length - 1);
	},
